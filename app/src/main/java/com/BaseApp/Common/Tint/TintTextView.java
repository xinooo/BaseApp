...
public class TintTextView extends TextView {
    ...
    private void obtainAttributeSet(Context c, AttributeSet attrs) {
        ...
        dayNormalColor = mNormalColor = a.getColor(R.styleable.TintView_text_normal_color, 0);
        mSelectColor = a.getColor(R.styleable.TintView_text_select_color, 0);
        // round = a.getInt(R.styleable.TintView_click_type, 2);
        if (mNormalColor != 0) {
    ...
    public void onNightMode(boolean isNight) {
        ...
        int nightColor = Color.parseColor("#6BB4BE");

        if (isNight) {
            dayNormalColor = mNormalColor;
            mNormalColor = whiteColor;
        } else {
            mNormalColor = nightColor;
        }
        setTextColor(mNormalColor);
    }
}
